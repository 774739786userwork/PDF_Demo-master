...
public class SavePdf {
    ...
    public void addText() {
        try {
            ...
            img.setAlignment(1);
            //设置Image图片大小，需要根据屏幕的分辨率，签名时PDF的放大比例来计算；还有就是当PDF开始显示的时候，他已经做了一次缩放，可以用 rectangle.getWidth() / (bitmap.getWidth() / 2)求得那个放大比
            img.scaleAbsolute(663 * 1.0f * density / 2 / scale * rectangle.getWidth() / (bitmap.getWidth() / 2), 1057 * 1.0f * density / 2 / scale * rectangle.getWidth() / (bitmap.getWidth() / 2));

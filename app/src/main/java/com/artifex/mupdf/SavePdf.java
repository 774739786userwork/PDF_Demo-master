...
public class SavePdf {
    ...
    public void addText() {
        try {
            ...
            img.setAlignment(1);
            //663，1057设置Image图片大小，需要根据屏幕的分辨率，签名时PDF的放大比例来计算；还有就是当PDF开始显示的时候，这里已经做了一次缩放，可以用 rectangle.getWidth() / (bitmap.getWidth() / 2)求得那个放大比
            img.scaleAbsolute(663 * 1.0f * density / 2 / scale * rectangle.getWidth() / (bitmap.getWidth() / 2),
                    820 * 1.0f * density / 2 / scale * rectangle.getWidth() / (bitmap.getWidth() / 2));
            //这里设置image相对PDF左下角的偏移量，得到放大后位置相对于整个PDF的百分比再乘PDF的大小得到他的相对偏移位置

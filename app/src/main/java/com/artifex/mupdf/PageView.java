...
public abstract class PageView extends ViewGroup {
	...
	private       BitmapHolder mEntireBmh; //新增修改代码
	private       BitmapHolder mPatchBmh;	//新增修改代码

	private       AsyncTask<Void,Void,Bitmap> mDrawEntire;//新增修改代码
	private       AsyncTask<Void,Void,LinkInfo[]> mGetLinkInfo;//新增修改代码
	...
	public PageView(Context c, Point parentSize) {
		...
		mEntireBmh = new BitmapHolder();//新增修改代码
		mPatchBmh = new BitmapHolder();//新增修改代码
	...
	public void blank(int page) {
		...
		if (mDrawPatch != null) {
			mDrawPatch.cancel(true);//新增修改代码
			mDrawPatch = null;
		}

		if (mGetLinkInfo != null) {
			mGetLinkInfo.cancel(true);//新增修改代码
			mGetLinkInfo = null;
		}
	...
	public void setPage(int page, PointF size) {
		...
		mDrawEntire = new AsyncTask<Void,Void,Bitmap>() {
			...
			protected Bitmap doInBackground(Void... v) {
				return drawPage(mSize.x*1, mSize.y*1, 0, 0, mSize.x*1, mSize.y*1);
			}
			protected void onPreExecute() {
//				progressDialog = ProgressDialog.show(mContext,null,"正在载入");
				PageView.this.setFocusable(false);
				mEntire.setImageBitmap(null);
...
//				progressDialog.dismiss();
				PageView.this.setFocusable(true);
		...
		if (viewArea.width() != mSize.x || viewArea.height() != mSize.y) {
			...
			mDrawPatch = new AsyncTask<PatchInfo,Void,PatchInfo>() {
				...
				protected void onPostExecute(PatchInfo v) {
					...
					if (v.bm != null) {
						mPatch.setImageBitmap(v.bm);//新增修改代码
						v.bmh.setBm(v.bm);
						v.bm = null;
					}
					//requestLayout();
